...
Тут эксперимент с HUGO + шаблон PaperMod (https://adityatelange.github.io/hugo-PaperMod/).

Шаблон копи-паст ручками на комп.

  С чем столкнулись при использовании этого шаблона:

Собирался с ошибкой при uglyURLs: true - schema_json.html неправильно генерит html, пропускает запятую между блоками. 

"Некрасивые" урл ставил для хотел уменьшить количество вложенных уровней, т.к. на narod.ru ограничение по глубине: "552 Quota on directory nesting level exceeded (4 levels is max): page won't be created". 


В итоге для сборки без ошибки нашел решение DeepSeek: 
> Исправьте генерацию JSON в шаблоне schema_json.html. Чтобы окончательно устранить проблему, замените содержимое файла themes/PaperMod/layouts/partials/templates/schema_json.html на следующий упрощённый вариант (без BreadcrumbList, только базовые типы). Это гарантирует корректный JSON и не нарушит работу сайта.
