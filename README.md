...
Создание статического сайта для замены/обновления уже существующего на narod.ru статического персонального сайта учителя математики.
 
## Итог ##

В данном варианте сайта используем совсем не новую версию HUGO и тему PaperMod (ограничения для совместимости с ОС старенького mac mini).

Все изменения выполнены в конфигурационных и других файлах Hugo (layouts/, assets/) и не затрагивают саму тему PaperMod, поэтому теоретически её дальнейшие обновления не сломают добавленный функционал:

- изменяется размер окна вывода при смене разрешения экрана или изменении размеров окна браузера (при достижении определенных значений высоты и ширины),
- строка меню прячется и включается гамбургер-меню при уменьшении размера окна,
- обложки постов могут динамически меняться при смене темы (светлая / тёмная),
- фото профиля на главной могут меняться при смене темы,
- изображения в тексте могут меняться при смене темы,
- математические формулы стабильно работают (MathJax),
- фавиконка может изменяться при смене темы.
...
> Создавайте сайт проекта с любым именем репозитория — это самый простой и гибкий вариант. После загрузки файлов и включения GitHub Pages ваш сайт будет доступен по адресу https://ваш-логин.github.io/имя-репозитория/. Все проблемы с глубиной вложенности, именами файлов и кодировками останутся в прошлом.
  ...
  Вот и пришли сюда - проверить возможности шаблонов на практике.
</div>

<div align="justify">
В теме PaperMod нет гамбургер-меню, в результате на узком экране смартфона смотрится не очень, сделал пока так:
</div>
